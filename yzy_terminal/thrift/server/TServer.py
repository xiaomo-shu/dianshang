...
class TThreadPoolServer(TServer):
    ...
    def serveClient(self, client):
        ...
        self.conn_mq.put([1, iprot])

        try:
            while True:
                self.processor.process(iprot, oprot)
        except TTransport.TTransportException as x:  # close socket can not catch???
            logger.warning('TTransport.TTransportException: {}'.format(x))
            pass
        except Exception as x:
            logger.warning('Exception: {}'.format(x))
            pass
            #logger.exception(x)
