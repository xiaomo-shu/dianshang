...
msg = b'\x01\x00\x01\x00)#\x00\x00\x01\x00\x00\x00+\x00\x00\x00\x01\x00\x01\x01\x00\x00\x00\x00{"mac": "11111-11113", "ip": "172.16.1.57"}'
msg2 = b'\x01\x00\x00\x00\x02\x00\x00\x00!\x1d\x00\x00=\x00\x00\x00\x01\x00\x01\x01\x08\x00\x00\x0012345678{"cmd": "template_disk_list", "data": {"mac": "11111-11113"}}'
msg3 = b'\x01\x00\x00\x00\x03\x00\x00\x00c\x13\x00\x00\x0e\x01\x00\x00\x01\x00\x01\x01\x08\x00\x00\x0012345678{"cmd": "compare_template", "data": {"mac": "11111-11113", "template": {"system_disks": ["voi_0_91f9d1ba-cb4a-41ba-971a-618f9e306571", "voi_1_91f9d1ba-cb4a-41ba-971a-618f9e306571"], "template_name": "voi_test1", "template_uuid": "f15a1759-789e-4e17-a3e1-e723121e9314"}}}'
heart_msg = b'\x01\x00\x01\x00(#\x00\x00\x01\x00\x00\x00\x16\x00\x00\x00\x01\x00\x01\x01 \x00\x00\x00%s{"mac": "11111-11113"}'
...
def main():
    msg = b'\x01\x00\x01\x00)#\x00\x00\x01\x00\x00\x00+\x00\x00\x00\x01\x00\x01\x01\x00\x00\x00\x00{"mac": "11111-11113", "ip": "172.16.1.56"}'
    s = socket(AF_INET, SOCK_STREAM)
    s.connect(('192.169.27.215', 50007))
    t = time.time()
    s.send(msg)
    h = s.recv(8192)
    print(h)
    ...
    heart_msg_str = heart_msg % token_bin
    ...
    while True:
        ...
        if service_code == 9000:
            ...
            print("发送种子文件： %s" % (data_size + token_len))
            ...
            with open("torrent_test_3.torrent", "wb") as f:
                f.write(torrent_file[66:])
            print("torrent_test write success !!!!!")
